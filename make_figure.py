...
# mytable
# # %%
# mytable = TableOne(test, columns=cols, categorical=cols,
#                    groupby='tilt', nonnormal='bili')
# mytable
# %%
...
metric_df_dual = pd.DataFrame(data=model_dicts_dual)[
    ['training_data', 'dataset', 'model_name', 'accuracy', 'precision', 'f1_score', 'auc']].sort_values(by=['training_data', 'dataset'])
...
# %%
dict_ls = []
for model in ['VGG16', 'VGG19', 'DenseNet121']:
    for dataset in ['All', 'Tilt', 'Non_Tilt']:
        loss_idx = df.query("model == '%s' and dataset_name == '%s'" % (
            model, dataset))['loss_mean'].idxmin()
        acc_idx = df.query("model == '%s' and dataset_name == '%s'" %
                           (model, dataset))['acc_mean'].idxmax()
        dict_ls.append({
            'model': model,
            'dataset': dataset,
            'best_loss': str(round(df['loss_mean'].loc[loss_idx], 4)) + '±' + str(round(df['loss_std'].loc[loss_idx], 4)),
            'best_accuracy': str(round(df['acc_mean'].loc[acc_idx], 4)) + '±' + str(round(df['acc_std'].loc[acc_idx], 4))
        })
